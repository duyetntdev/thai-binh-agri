...
<section class="max-w-4xl mx-auto px-4 py-10">
    <div class="bg-white rounded-3xl border border-gray-200 p-8 shadow-sm">
        <h1 class="text-3xl font-bold text-gray-900 mb-4">Chỉnh sửa nội dung trang</h1>
        ...
        <form action="{{ route('admin.pages.update', $page) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="content" rows="10"
                          class="mt-2 w-full rounded-3xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700">{{ old('content', $page->content) }}</textarea>
                @error('content')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>

            <div class="flex items-center gap-3">
                <input id="is_active" type="checkbox" name="is_active" {{ $page->is_active ? 'checked' : '' }} class="h-4 w-4 text-green-600 border-gray-300 rounded" />
